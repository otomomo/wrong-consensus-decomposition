...
\shorttitle{Decomposing Wrong-Consensus Agreement in LLM Self-Consistency}
\shortauthors{Zhang, Tang, Long, Tang, and Luo}

\title[mode=title]{Decomposing Wrong-Consensus Agreement in Large Language Model Self-Consistency}

\author[1,2]{Lizhuo Zhang}[orcid=0009-0004-0749-3239]
\ead{zhanglizhuo@hunau.edu.cn}
\credit{Conceptualization, Methodology, Software, Formal analysis, Investigation, Data curation, Visualization, Writing---original draft, Writing---review and editing, Supervision}

\author[1]{Mengmeng Tang}
\credit{Software, Formal analysis, Investigation, Data curation, Visualization, Writing---original draft}

\author[1,2]{Chenfeng Long}\cormark[1]
\ead{elong@hunau.edu.cn}
\credit{Writing---review and editing, Supervision, Resources, Funding acquisition}

\author[2,3]{Xiaoyong Tang}
\credit{Writing---review and editing, Supervision, Resources, Funding acquisition}

\author[4]{Xiang Luo}
\credit{Writing---review and editing, Resources, Funding acquisition}

\affiliation[1]{organization={College of Information and Intelligence, Hunan Agricultural University},
    city={Changsha},
    postcode={410128},
    country={China}}

\affiliation[2]{organization={Yuelushan Laboratory},
    city={Changsha},
    postcode={410128},
    country={China}}

\affiliation[3]{organization={School of Computer Science and Technology, Changsha University of Science and Technology},
    city={Changsha},
    postcode={410114},
    country={China}}

\affiliation[4]{organization={Information Department, China Guangdong Tobacco Meizhou Ltd},
    city={Meizhou},
    postcode={514000},
    country={China}}
...
\nonumnote{This work was supported in part by the Yuelushan Laboratory Breeding Program under Grant YLS-2026-ZY01002, in part by the National Natural Science Foundation of China under Grant 62372064, and in part by the Meizhou Tobacco Science Research Project under Grant 202404.}
...
as strongly as right ones. This paper gives a quantitative account of
what that agreement contains. A pluralistic agreement index
$\agreementIndex$ normalizes a wrong run's expected agreement with the
consensus by the reference scale $d=(1-p)/(C-1)$ and is split into a
\emph{mechanical} component, what a vote would deliver given only a
per-case answer preference, and a \emph{preference-unexplained} residual.
The mechanical reference is leak-free: each case's preference and accuracy
are estimated from its other runs only. On the public GPT-4.1 per-run data
...
GPQA-Diamond against $0.59$--$0.78$ on open-domain AIME, where a residual
of $1.54$--$2.80$ $\agreementIndex$ units survives and is more than absorbed by a run-level
preference-heterogeneity reference. A controlled
replication under one fixed protocol (four runs per question, $K=32$
votes) on five open-weights checkpoints finds near-complete mechanical
coverage in all ten cells ($\phi\approx1$), robust to a two-run design; the
largest cell (\texttt{qwen3.5-122b}, $p=0.222$) sits inside the GPT-4.1 AIME
range and still saturates ($\phi=1.041$). A cross-system contrast at
comparable aggregate accuracy thus contrasts near-complete mechanical
agreement in the open-weights models against a larger residual in the
frontier family; the gap is not explained by benchmark identity or
single-sample accuracy alone but is confounded with the sampling protocol,
so family and protocol cannot be separated. Agreement is graded evidence,
not certification; no new voting method is proposed, and code and evidence
are committed.
\end{abstract}

\begin{highlights}
\item Wrong-consensus agreement is decomposed into a mechanical term and a residual.
\item The mechanical reference is leak-free: each case uses its other runs only.
\item Frontier coverage is 0.81-0.93 on multiple-choice vs 0.59-0.78 on open-domain math.
\item Five open-weights models saturate mechanically under one fixed protocol.
\item Agreement is graded evidence, not certification; no new voting method is proposed.
\end{highlights}

\begin{keywords}
large language models \sep majority voting \sep self-consistency \sep agreement decomposition \sep counterfactual analysis \sep uncertainty estimation
\end{keywords}
...
\end{equation}
where $\agreementIndex_{\mathrm{rival}}$ is a
\emph{leak-free} reference granting each case its per-case option
...
$K$ & samples per run (50 for gpt-4.1; 32 for Tier-3; 16/32 for Qwen MMLU) \\
...
\begin{table}[t]\begin{minipage}{\linewidth}
\centering
\caption{Per-cell sample sizes. Cases = distinct (question, model, prompt) instances; runs = $K=50$ sampling passes; wrong runs = runs whose plurality label is wrong (the effective sample for $\agreementIndex_{\mathrm{emp}}$); test runs = held-out wrong runs entering the rival null and $\agreementIndex_{\mathrm{emp}}^{(t)}$ (Section~\ref{sec:iid}).}
\label{tab:samples}
...
and the analysis can be regenerated from the raw files
 by the commands in each script's header. The funders had no role in study design, data collection
